...
def chat(model, chat_text, father_robot: ROBOT):
    ...
    controller_request.clear()
    controller_request.append(controller_ask)
    controller_return = ""
    ...
    try:
        if "转" in chat_text:
            # 请求运动控制输出
            body_controller = chat_comp.do(model="ERNIE-3.5-4K-0205", messages=controller_request, stream=False,
        ...
        # 请求其他输出
        temperature = father_robot.name if father_robot is not None else 0.8
        top_p = father_robot.name if father_robot is not None else 0.8
...
if __name__ == '__main__':
    while 1:
        question = input("请输入：\n")
        chat("ERNIE-Bot-turbo", chat_text=question, father_robot= None)
